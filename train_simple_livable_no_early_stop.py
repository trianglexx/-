...
def setup_training_environment():
    ...
    # 创建输出目录
    output_dir = Path("simple_livable_no_early_stop_results")
...
def save_results(model, training_history, test_results, output_dir):
    """保存训练结果"""
    # 保存模型
    model_path = output_dir / "best_simple_livable_no_early_stop_model.pth"
    torch.save(model.state_dict(), model_path)
    logger.info(f"💾 模型已保存: {model_path}")

    # 保存训练历史
    history_path = output_dir / "simple_livable_no_early_stop_training_history.json"
    with open(history_path, 'w') as f:
        json.dump(training_history, f, indent=2)
    logger.info(f"📊 训练历史已保存: {history_path}")

    # 保存测试结果
    results_path = output_dir / "simple_livable_no_early_stop_final_results.json"
